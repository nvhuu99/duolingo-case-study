...
func (ps *PercentileStrategy) UseSource(src ReducedDataPoints) {
	ps.source = src
}

func (ps *PercentileStrategy) Make(bucket int64, dp []*DataPoint) (*DataPoint, error) {
	if len(dp) == 0 {
	...
	for i, point := range dp {
		...
		timestamps[i] = point.GetTimestamp().UnixMilli()
	}
	...
	return NewDataPoint(time.UnixMilli(medianTs), percentileValue), nil
}
